...
void RobotLog::deploy_button_clicked() {
	...
	filename_dialog.run();
	string robot_path_name = filename_box.get_text();
	string tmp_path_name = Glib::get_tmp_dir() + "/" + robot_path_name;
	...
	if (scp_status == 0) {
		...
		Gtk::MessageDialog fail_dialog("Deploy failed, scp returned " + to_string(scp_status));
		fail_dialog.run();
	}
	std::remove(tmp_path_name.c_str());
}

void RobotLog::export_path(string filename) {
	ofstream outcsv(filename);
